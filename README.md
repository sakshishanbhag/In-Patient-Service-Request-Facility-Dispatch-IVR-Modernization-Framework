...
pip install -r requirements.txt || pip install fastapi uvicorn twilio python-dotenv pytest
```
...
3. Run server and expose locally for Twilio:
...
4. Configure Twilio phone number webhook (Voice) to `https://<ngrok-id>.ngrok.io/voice`.

5. Trigger a test call (`python trigger_call.py`) or call the Twilio number directly.

## Notes & Next Steps

- Replace in-memory session store with Redis for production readiness.
- Add audit logging and persistent storage for requests.
- Add rate-limits and ACLs to control access to dispatch endpoints.
- Add a short demo GIF or a hero image (place under `assets/hero.png`) to match the repository visual from your attachment.

---

If you'd like, I can now:
- add `.env.example` and `requirements.txt`
- create `assets/hero.png` placeholder and add badges or a GIF
- generate a concise `CONTRIBUTING.md`

Tell me which and I'll add them next.
---
